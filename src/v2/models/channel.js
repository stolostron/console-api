    ...
    const result = await Promise.all(resources.map((resource) => {
      ...
      if (channelKinds[resource.kind] === 'channels') {
        return this.kubeConnector
          .post(`/apis/app.ibm.com/v1alpha1/namespaces/${namespace}/channels`, resource)
          .catch(err => ({
            status: 'Failure',
            message: err.message,
          }));
  ...
  async getChannelOverview(name, namespace = 'default') {
    ...
    if (name) {
      chs = await this.kubeConnector.getResources(
        ns => `/apis/app.ibm.com/v1alpha1/namespaces/${ns}/channels/${name}`,
        { namespaces: [namespace] },
      );
    } else {
      chs = await this.kubeConnector.getResources(ns => `/apis/app.ibm.com/v1alpha1/namespaces/${ns}/channels`);
    }
