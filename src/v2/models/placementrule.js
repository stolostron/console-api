    ...
    const result = await Promise.all(resources.map((resource) => {
      ...
      if (placementruleKinds[resource.kind] === 'placementrules') {
        return this.kubeConnector
          .post(`/apis/app.ibm.com/v1alpha1/namespaces/${namespace}/placementrules`, resource)
          .catch(err => ({
            status: 'Failure',
            message: err.message,
          }));
  ...
  async getPlacementRuleOverview(name, namespace = 'default') {
    ...
    if (name) {
      chs = await this.kubeConnector.getResources(
        ns => `/apis/app.ibm.com/v1alpha1/namespaces/${ns}/placementrules/${name}`,
        { namespaces: [namespace] },
      );
    } else {
      chs = await this.kubeConnector.getResources(ns => `/apis/app.ibm.com/v1alpha1/namespaces/${ns}/placementrules`);
    }
